...
export default function SpaceMissionForm() {
  ...
  // при первом рендере и при любых изменениях - update
  useEffect(() => {
    console.log("Use effect in Space Mission - no dependencies");
  });
  //! useEffect с зависимосятми
  // при первом ренедрe - mount или когда менЯется указаная в массиве переменная - name
  useEffect(() => {
    console.log("Use effect in Space Mission - with dependencies on name");
  }, [name]);
